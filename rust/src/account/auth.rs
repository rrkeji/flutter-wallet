use anyhow::anyhow;
use bytes::Bytes;
use keys_idns_api::idns::account::{
...
impl AuthServiceImpl {
    ...
    pub async fn is_online(&self) -> Result<bool> {
        ...
        if let Ok(_) = keys_idns_api::kvstore::KVStore::get_value(&token).await {
            Ok(true)
        } else {
            Ok(false)
        }
    ...
    pub async fn unimport(&self) -> Result<bool> {
        //TODO 检查是否有数据未保存
        keys_idns_api::utils::files::file_delete("", "account.json")?;
    ...
    pub fn is_imported(&self) -> Result<bool> {
        //查看文件是否存在
        let exist = keys_idns_api::utils::files::file_exists("", "account.json")?;
        if !exist {
            return Ok(false);
        }

        //判断是否有account.json文件
        let json_str = keys_idns_api::utils::files::read_string_from_file("", "account.json")?;
    ...
    pub async fn login_by_password(&self, password: &String) -> Result<IdnsToken> {
        ...
        let user_phrase = keys_idns_api::get_user_phrase(password)?;
        identities_idns_api::set_external_api_identity_signature(
            keys_idns_api::get_signature_nonce(&user_phrase)?,
        );
        //
    ...
    pub async fn import_and_login(
            ...
            encrypt_message_impl(&phrase, password.as_str())
        {
            ...
            if update_file {
                //保存文件成功之后在保存密码到内存中
                if let Ok(_) = keys_idns_api::utils::files::write_to_file(
                    "",
                    "account.json",
    ...
    pub async fn login_by_identity(&self, identity: &String) -> Result<IdnsToken> {
        //获取到identity的信息
        let identity_entity = crate::identity::IdentityServiceImpl::new()
    ...
    pub async fn user_import_and_login(
            ...
            encrypt_message_impl(&phrase, password.as_str())
        {
            ...
            if let Ok(_) = keys_idns_api::utils::files::write_to_file(
                ...
                identities_idns_api::set_external_api_identity_signature(
                    keys_idns_api::get_signature_nonce(phrase)?,
                )?;
                return Ok(token.clone());
            }
        }
