...
impl AttributeServiceImpl {
    ...
    pub async fn attribute_list(&self, source_identity: &String) -> Result<Vec<AttributeEntity>> {
        let res = identities_idns_api::identity::IdnsAttribute::attribute_list(source_identity).await?;
        Ok(res)
    }

    pub async fn query_one_attribute(&self, id: &String) -> Result<AttributeEntity> {
        let res = identities_idns_api::identity::IdnsAttribute::query_one_attribute(id).await?;
        Ok(res)
    }
    pub async fn create_attribute(&self, request: &AttributeCreateRequest) -> Result<bool> {
        let _ = identities_idns_api::identity::IdnsAttribute::create_attribute(request).await?;
        Ok(true)
    }
    pub async fn remove_attribute(&self, id: &String) -> Result<bool> {
        let _ = identities_idns_api::identity::IdnsAttribute::remove_attribute(id).await?;
        Ok(true)
    }
    pub async fn update_attribute(&self, request: &AttributeUpdateRequest) -> Result<bool> {
        let _ = identities_idns_api::identity::IdnsAttribute::update_attribute(request).await?;
        Ok(true)
    }
}
